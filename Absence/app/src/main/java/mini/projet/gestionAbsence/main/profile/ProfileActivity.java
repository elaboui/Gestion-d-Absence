...
public class ProfileActivity extends AppCompatActivity {
    DatabaseHandler handler = AppAcceuil.handler;
                ...
                alert.setPositiveButton("oui", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (AppAcceuil.handler.execAction(qu)) {
                            Log.d("delete", "fait par un étudiant");
                            String qa = "DELETE FROM ATTENDANCE WHERE register = '" + regno.toUpperCase() + "'";
                            if (AppAcceuil.handler.execAction(qa)) {
                                Toast.makeText(getBaseContext(), "Deleted", Toast.LENGTH_LONG).show();
                                Log.d("delete", "fait de la présence");
                            }
                        }
                    }
                });
    ...
    public void find(View view) {
        ...
        if (cur != null && cur1 != null) {
            ...
            try {
                ...
                if (att <= 0)
                    att = 0f;
                Log.d("ProfileActivity", "Totale = " + cur.getCount() + " profiter = " + cur1.getCount() + " par " + att);
            } catch (Exception e) {
                att = -1;
            }
        }
        ...
        if (cursor == null || cursor.getCount() == 0) {
            ...
            String attendance = "";
            if (att < 0) {
                attendance = "Participation non disponible";
            } else
                attendance = "Présence " + att + " %";
            cursor.moveToFirst();
            String buffer = "";
            ...
            if (cursorx == null || cursorx.getCount() == 0) {
                ...
                while (!cursorx.isAfterLast()) {
                    ...
                    dates.add(cursorx.getString(0) + ":" + cursorx.getInt(1) +"l'heure");
